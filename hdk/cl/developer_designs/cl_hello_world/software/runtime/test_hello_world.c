...
#define Write_Reg_ADDR	UINT64_C(0x50c)
...
int main(int argc, char **argv) {
    ...
    printf("What we want to do is the Vector-Vector Add in the Systolic Array");
...
int peek_poke_example(int slot_id, int pf_id, int bar_id) {
    ...
    uint32_t i, value;
    srand(3);
    uint32_t vec_A[Vector_Size];
    uint32_t vec_B[Vector_Size];
    uint32_t vec_Result[Vector_Size];
    ...
    for (i = 0; i < Vector_Size; i++){
        ...
        printf("0x%x", vec_Result[i]); 
   }
   
    uint32_t state = 0;
    for (i = 0; i < Vector_Size; i++){
        rc = fpga_pci_poke(pci_bar_handle, HELLO_WORLD_REG_ADDR, vec_A[i]);
        fail_on(rc, out, "Unable to write to the fpga !");
        rc = fpga_pci_peek(pci_bar_handle, Write_Reg_ADDR, &state);
    ...
    for (i = 0; i < Vector_Size; i++){
        rc = fpga_pci_poke(pci_bar_handle, HELLO_WORLD_REG_ADDR, vec_B[i]);
        fail_on(rc, out, "Unable to write to the fpga !");
        rc = fpga_pci_peek(pci_bar_handle, Write_Reg_ADDR, &state);
        fail_on(rc, out, "Unable to read from fpga");
        printf("write %x to data BRAM  \n", i);
    ...
    /* read result back and print it out; you should expect the byte order to be
     * reversed (That's what this CL does) */
